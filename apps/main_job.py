...
def main(session=None, csv_path="/opt/spark/project/data/input/sample.csv"):
    ...
    - Silver: cleans the `name` field, and merges (upserts) into `local.db.silver_users` keyed by `id`.
    ...
    try:
        ...
        if not spark.catalog.tableExists("local.db.bronze_events"):
            empty_df = spark.createDataFrame([], EXPECTED_SCHEMA)
            empty_df.writeTo("local.db.bronze_events").using("iceberg").create()
        ...
        # Keep the latest row per id (using insertion order)
        window_spec = (
